...
// ------------------------------
void setupWiFi() {
  pinMode(BUTTON_PIN, INPUT_PULLDOWN); // botão envia 3.3V quando pressionado
  delay(100);
  ...
  if (digitalRead(BUTTON_PIN) == HIGH) {  // agora o botão é HIGH quando pressionado
    delay(100);
  ...
  if (startAP) {
    ...
    wm.resetSettings(); // comente até testar o botão
